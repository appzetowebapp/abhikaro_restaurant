...
class _WebViewScreenState extends State<WebViewScreen> with WidgetsBindingObserver {
  static const platform = MethodChannel('com.abhikaro.restaurant/geolocation');
  InAppWebViewController? _webViewController;
  bool _isLoading = true;
  double _loadingProgress = 0.0;
  ...
  Future<void> _showOverlayWindow() async {
    ...
    await FlutterOverlayWindow.showOverlay(
      enableDrag: true,
      overlayTitle: "Abhikaro Restaurant Overlay",
  ...
  Future<void> _handleBlobDownload({
    ...
    try {
      ...
      final resultMap = await completer.future.timeout(
        const Duration(seconds: 30),
        onTimeout: () {
          throw Exception('Timeout waiting for blob data (CSP restrictions might be blocking extraction)');
        },
      );
                        ...
                        controller.addJavaScriptHandler(
                          ...
                          callback: (args) {
                            if (args.isNotEmpty && args[0] is Map) {
                              ...
                              final String? base64Data = data['data']?.toString();
                        ...
                        try {
                          ...
                          if (result.success && result.filePath != null) {
                            ...
                            try {
                              await NotificationService().showSimpleNotification(
                                title: 'Download Complete',
                                body: 'File saved: ${result.filename ?? "File"}',
